...
So, let's first create an push the other two branches.
...
`-r` indicates the GitHub repsitory we will be pushing too. It also leads us nicely to the next and final configuration we need to perform - getting a GitHub access token.
...
Forgetting to declare the key for GH_TOKEN
Using a whole new repository for development

## Conclusion

Secrets and accessing them in GH Actions
